package io.infinitestrike.flatpixel.compat;
...
public class Loader {
	...
	public static LoaderResult readFile(File f) throws FileNotFoundException {
		String settingsFile = TextFromFile.getText(f);
		...
		for (String line : lines) {
			...
			line = line.replace(";", ""); // remove delimiter.
			// =================================================
			...
			if(!typeSplit[1].contains("=")) {
				Console.Log("SyntaxError on line: " + currentLine + ". No '=' Delimiter in key-value pair.");
				Console.Log(line);
				Console.Log("^");
				return null;
			}
	...
	public static boolean getValueBoolean(LoaderEntry e) {
		if (e.entryType == LoaderType.BOOL) {
			try {
				...
				Console.Error("Loaded Value-Type Was Not Bool.", ex);
			}
		}
	...
	public static int getValueInt(LoaderEntry e) {
		try {
			...
			Console.Error("Loaded Value-Type Was Not Integer.", ex);
		}
	...
	public static int getValueHex(LoaderEntry e) {
		try {
			...
			Console.Error("Loaded Value-Type Was Not Hex.", ex);
		}
	...
	public static long getValueLong(LoaderEntry e) {
		try {
			...
			Console.Error("Loaded Value-Type Was Not Long.", ex);
		}
	...
	public static float getValueFloat(LoaderEntry e) {
		try {
			...
			Console.Error("Loaded Value-Type Was Not Float.", ex);
		}
	...
	public static String getValueOption(LoaderEntry e, LoaderResult source) {
		if(e.entryType == LoaderType.OPTION) {
			...
			try {
				...
				Console.Error("Error Occurred while parsing elements: ", ex);
			}
		}
